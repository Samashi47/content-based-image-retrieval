...
from logic.search_helpers import process_query_image, SimpleSearch, AdvancedSearch, RelevanceFeedbackSearch
...
def advanced_search():
    ...
    response_dict.append({'weights': weights, 'query_id': str(_id)})
...
def relevance_feedback():
    ...
    if query_desc.count() == 0:
        return Response(json.dumps({'message': 'Query not found'}), mimetype='application/json', status=404)
    
    
    individual_sims = sorted(
        [sim for sim in query_desc[0]['individual_sims'] if sim['image_name'] in top_image_names],
        key=lambda sim: top_image_names.index(sim['image_name'])
    )
    
    top_images, weights, individual_sims = RelevanceFeedbackSearch(query_desc, feedback_data, individual_sims, weights, n=15)
    
    collection.update_one({'_id': ObjectId(query_id)}, {'$set': {'individual_sims': individual_sims}})
    
    response_dict = [
        {'title': img_path.replace("RSSCN7/", ""), 'image': encode_image_to_base64(img_path), 'similarity': sim} for img_path, sim in top_images
    ]
    response_dict.append({'weights': weights, 'query_id': str(query_id)})
